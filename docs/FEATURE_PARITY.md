...
| Clipboard | `Clipboard.swift`, `ClipboardStrip.swift`: text, links, RTF, images and file references; floating horizontal strip, full library, search/type/tag filters, editable tags, hex/RGB color previews, pins, source-app badges, duplicates, copy/direct paste, pause. | Image OCR indexing, sync and additional reference layouts. Concealed data filtering depends on source apps marking content. |
